...
public class Baby : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.tag == "Player" && pc.pounceCooldown - pc.pCDTimer <= pc.pounceDuration) //collision with player while pouncing
        {
            ...
            transform.localPosition = new Vector3(carryOffset.x, 1.5f, carryOffset.y); // set local position to be appropriately placed on the parent's back
            gameObject.tag = "Carried"; // update tag so player doesn't constantly point at the baby on its back
            other.gameObject.GetComponent<PlayerControl>().startTeleportTimer(); // teleport player
        }
    }
}
